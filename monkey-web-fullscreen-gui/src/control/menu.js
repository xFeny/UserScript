...
export default {
  initMenuCmds() {
    if (FyTools.isExecuted("hasMenu", window) || !FyTools.isTopWin()) return;
    this.setupMenuChangeListener();
    this.setupMenuCmds();
  },
  setupMenuChangeListener() {
    [Store.NOT_SUPPORTED.name, Store.ENABLE_NANO.name + this.host].forEach((key) =>
      GM_addValueChangeListener(key, () => this.setupMenuCmds())
    );
  },
  setupMenuCmds() {
    const isHide = !FyTools.hasMoveBefore();
    const enPip = `${Store.NOT_SUPPORTED.get() ? "启" : "禁"}用文档画中画`;
