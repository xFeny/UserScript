...
不是链接式会查找不到剧集列表，如：<a href="https://ddys.pro" target="_blank">低端影视</a>；或当前播放集数是不可点击状态，如：<a href="https://www.56.com/" target="_blank">56网</a>，就会切换下集失败。
...
如果验证提示 `获取不到当前集数` ，手动编辑文本框的选择器。验证成功成功后保存。

###### 2. 抓取所有集数列表的某个集数所在位置

抓取到当前集数的元素后，继续键盘同时按住 `ctrl` + `alt` + 鼠标点击所有集数列表所在的其中一个集数(最好不要再点击当前集数)，弹窗验证元素、确认保存。

###### 3. 重新抓取元素

要重新抓取需要在脚本菜单中 `删除此站点的剧集选择器` 。
