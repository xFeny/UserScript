...
当「自动切换下集」功能启用后，若希望**特定网址(如 B站非番剧页面)** 不触发自动切换，可将该网址添加至忽略列表。
...
默认支持 `Ctrl + [1-5]` 组合键，若需更多预设值，可修改以下代码：
...
默认在全屏模式下，界面右上角将自动展示系统时间、视频剩余时长和进度百分比，方便查看当前时间与视频播放进度。若无需显示内容，可直接在设置中禁用该功能，避免多余信息干扰。
