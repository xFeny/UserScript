...
##### 忽略网址
 ...
 `此站启用自动网页全屏` 时，是对整个网站的域名起作用的，有些网站会有鼠标移入预览功能（如 `YouTube` 首页），此时会自动进入网页全屏，可是又不希望其网页全屏，此时可以将网址添加到忽略列表中。
...
启用自动切换下集后，如不希望对某个网址自动切换下集时（如 B站普通页合集，默认已添加），可以将网址添加到忽略列表中。
