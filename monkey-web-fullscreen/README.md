...
> ⚠️ 注意：配置单个网站时须用 `@include`，而非 `@match`（`@match` 触发的是网站自带的图标，如果配置成 `@match` ，还需要在`IconsSelector` 定义相关图标的 `CSS` 选择器）。
...
脚本默认 `@match` 的网站，切换下集是点击其相关的图标元素进行切换。对于其他的视频网站，脚本提供了较为通用的切换下集功能，适用于绝大部分网站。
