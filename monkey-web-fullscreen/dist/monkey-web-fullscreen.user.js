...
(async function (notyf, Swal) {
  ...
  const SiteIcons = await (async () => {
    let remoteSelector = {};
    try {
      const res = await _GM.xmlHttpRequest({ url: "https://gitee.com/xfeny/static-files/raw/master/IconsSelector.json" });
